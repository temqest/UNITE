...
export default function EditEventModal({ isOpen, onClose, request, onSaved }: EditEventModalProps) {
  ...
  useEffect(() => {
    ...
    const category = event.categoryData || request.categoryData || request.category || {};
  ...
  const rawStart = request?.event?.Start_Date || request?.Start_Date || request?.StartDate || null;
  const displayStartDate = rawStart ? new Date(rawStart).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '';
